...
How it works
The frontend sends either raw article text or a URL to the /summarize endpoint.
If a URL is given, the backend resolves any redirects, fetches the page with browser-like headers, and extracts the clean article text.
The article text is sent to an LLM with a structured schema (via tool calling) that forces the response into a strict shape — no more hoping the AI's JSON is well-formed.
The backend validates the AI's chosen topic against a fixed list of 20 categories, silently correcting it if it doesn't match (e.g. "Movies" becomes "Entertainment").
If the AI call fails, the backend automatically retries once before returning a clean error message.
Running it locally
1. Clone the repo
git clone https://github.com/armaaaann7/Newsbite.git cd Newsbite
...
GROQ_API_KEY=your_groq_api_key_here GROQ_MODEL=openai/gpt-oss-120b
